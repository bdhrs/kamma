...
**Spec gate — files on disk before any code.** `spec.md` and `plan.md` must already exist on disk for the thread before you edit a single line of code. **Starting work with no thread at all is blocked, not permitted** — that is the case this gate exists for. If either file is missing, or no thread exists, run `/kamma:1-plan` to create one first (or, for a tiny self-contained change, `/kamma:quick`, which is the no-spec path by design). Never decide to skip the ceremony silently. Implementing first and backfilling the thread files afterwards is dishonest and is not allowed, even for a 3-line change.

**The spec gate covers shell commands, not just file edits.** Bulk or destructive writes to a live external system — `gh` write subcommands, `gh api` carrying a write method or a graphql `mutation`, `curl` with POST/PUT/PATCH/DELETE, a package publish — are gated exactly like a file edit. The reason is an incident: 213 GitHub issues were migrated, and 19 labels deleted, with no thread and no spec, because every action went through the shell and the gate only watched file-edit tools. A file edit is recoverable from git; a bulk write to a live system is recoverable only from a snapshot nobody took. Edits inside `kamma/` are always allowed, so writing the thread that unblocks the gate always works.
